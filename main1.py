# pure alpha beta pruning, serves as a baseline for testing purposes 
import pygame
import sys
...
def evaluate_position(lines_drawn, squares):
    # Simple evaluation: count completed squares for each player
    current_edges = extract_edge_set(lines_drawn)
    player1_squares = 0
    player2_squares = 0
    ...
    for square in squares:
        edges = edges_of_square(square)
        if all(edge in current_edges for edge in edges):
            # Determine which player completed this square
            for line in lines_drawn:
                if line['id1'] in square and line['id2'] in square:
                    if line['player'] == 1:
                        player1_squares += 1
                    else:
                        player2_squares += 1
    ...
    return player1_squares - player2_squares
