...
from test2 import start_display
...
class DotAndBoxGame:
    def __init__(self, root):
        ...
        self.root.iconbitmap("dotsandboxes.ico")
    ...
    def create_main_menu(self):
        ...
        btn_person_vs_ai = tk.Button(container, text="Person vs AI", font=("VNI-Dom", 14),
                                     command=lambda: self.set_mode("Person vs AI"), bd="4", bg="light blue",
                                     fg="purple", width="10", pady="5")
        btn_person_vs_ai.grid(row=2, column=0)

        btn_ai_vs_ai = tk.Button(container, text="AI vs AI", font=("VNI-Dom", 14),
                                 command=lambda: self.set_mode("AI vs AI"), bd="4", bg="light green", fg="darkred",
                                 width="10", pady="5")
    ...
    def select_level(self):
        ...
        btn_easy = tk.Button(container, text="Dễ", font=("VNI-Dom", 14), command=self.select_board_size, bd="6",
                             bg="light pink", fg="blue")
        # btn_easy.pack(pady=20)
        btn_easy.grid(row=2, column=0)

        btn_hard = tk.Button(container, text="Khó", font=("VNI-Dom", 14), command=self.select_board_size, bd="6",
                             bg="light pink", fg="blue")
        # btn_hard.pack()
        ...
        for size in sizes:
            if (size == "3x3"):
                btn = tk.Button(container, text=size, font=("VNI-Dom", 14),
                                command=lambda s=size: self.initiate_game_start(s, self.selected_mode), bd="5",
                                bg="Green", fg="Yellow")
