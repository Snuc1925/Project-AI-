...
def bot_choose_move_MCTS(lines_drawn, squares, list_squares_1=[], list_squares_2=[], time_limit=1.0):
    ...
    current_lines_set = extract_edge_set(lines_drawn)

    possible_moves = []
    unique_potential_edges = set()
    for square_config in squares:
        edges = edges_of_square(square_config)
        for edge in edges:
            if edge not in current_lines_set:
                unique_potential_edges.add(edge)
    possible_moves = list(unique_potential_edges)

    if not possible_moves: return None

    for move in possible_moves: # MCTS bot also takes immediate wins
        if check_new_squares(move, lines_drawn, squares):
            return move

    safe_moves_for_mcts = [m for m in possible_moves if not is_dangerous_move(m, lines_drawn, squares)]
    exploration_candidates = safe_moves_for_mcts if safe_moves_for_mcts else possible_moves
    
    if not exploration_candidates:
         return random.choice(possible_moves) if possible_moves else None

    move_stats = {move: {"wins": 0, "sims": 0} for move in exploration_candidates}
    
    max_possible_lines = 0
    if squares: 
        all_unique_edges_in_game = set()
        for sq_cfg in squares:
            for edg in edges_of_square(sq_cfg):
                all_unique_edges_in_game.add(edg)
        max_possible_lines = len(all_unique_edges_in_game)
    else: # No squares, line game. Need rows/cols to calculate this, or pass it.
          # For now, assume this function is called where squares exist.
          # If squares is empty for 1xN or Nx1, this will be 0.
          # This needs a more robust way to get total lines if used for line games.
          # Let's assume for MCTS context, it's usually on a grid with squares.
          # If not, MCTS might not be the best if game end isn't based on square count.
        pass


    def simulate_game_from_move(initial_move, lines_drawn, squares_1, squares_2):
        ...
        # Player 2 (MCTS bot) makes the initial_move_for_mcts_bot
        lines_before = copy.deepcopy(lines_copy)
        lines_copy.append({"id1": initial_move[0], "id2": initial_move[1], "player": active_player})
        new_squares = check_new_squares(initial_move, lines_before, squares)
        ...
        # Simulation loop
        while len(extract_edge_set(lines_copy)) < max_possible_lines : # Use max_possible_lines if available
            if time.time() - start_time > time_limit * 1.1 : # Slightly more time for full simulation
                break

            move = None
            # In simulation, use the simpler bot for both players for speed
            if active_player == 1: 
                move = bot_choose_move(lines_copy, squares) 
            else: 
                move = bot_choose_move(lines_copy, squares) # Simpler bot for P2 in simulation too


            if not move: break 

            lines_before_1 = copy.deepcopy(lines_copy)
            lines_copy.append({"id1": move[0], "id2": move[1], "player": active_player})
            new_squares_sim = check_new_squares(move, lines_before_1, squares)

            if new_squares_sim:
                if active_player == 1: squares_1.extend(new_squares_sim)
                else: squares_2.extend(new_squares_sim)
            else:
                active_player = 3 - active_player 
        
        if len(squares_2) > len(squares_1): return 2 
        elif len(squares_1) > len(squares_2): return 1 
        else: return 0 

    sim_count = 0
    # Ensure MCTS runs for a minimum number of simulations if time permits, or cap iterations
    max_simulations = 200 # Example cap, adjust based on performance
    while (time.time() - start_time < time_limit and sim_count < max_simulations):
        if not exploration_candidates: break 
        
        move = random.choice(exploration_candidates) 
        winner = simulate_game_from_move(move, lines_drawn, list_squares_1, list_squares_2)
    ...
    if sim_count == 0 and exploration_candidates:
        return random.choice(exploration_candidates)

    best_move = None
    best_win_rate = -1.0
    for move in exploration_candidates: 
        stats = move_stats[move]
        if stats["sims"] > 0:
            win_rate = stats["wins"] / stats["sims"]
            if win_rate > best_win_rate:
                best_win_rate = win_rate
                best_move = move
            elif win_rate == best_win_rate and best_move is None:
                best_move = move       
    if best_move: return best_move
    
    # Fallbacks if MCTS yielded no clear best move or didn't run
    if exploration_candidates: return random.choice(exploration_candidates)
...
def display_dots(rows, cols, mode,on_back_to_menu = None): 
    ...
    total_possible_lines = 0
    if rows > 0 and cols > 0 :
        if rows > 1 and cols > 1 : # Có ô vuông
            total_possible_lines = (rows * (cols - 1)) + ((rows - 1) * cols)
        elif rows * cols > 1: # Lưới dạng đường thẳng (1xN hoặc Nx1, N>1)
            total_possible_lines = max(rows,cols) - 1
            if not squares: print("Note: Game on a line grid, no squares to form.")
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if is_human_player1 and current_player == 1 and not game_over:
        ...
        # Bot player 1
            if not game_over and current_player == 1 and not is_human_player1:
                time.sleep(1)
                ...
                if move:
                    ...
                    if new_squares_indices:
                        ...
                        current_player = 2
                elif len(lines_drawn) < total_possible_lines:
                        print(f"AI1 ({player1_name}) found no move! Lines: {len(lines_drawn)}/{total_possible_lines}")
                        game_over = True
                ...
                if move:
                    ...
                    if new_squares_indices:
                        ...
                        current_player = 1
                            ...
                            print(f"AI2 ({player2_name}) MCTS found no move! Lines: {len(lines_drawn)}/{total_possible_lines}")
                            game_over = True
